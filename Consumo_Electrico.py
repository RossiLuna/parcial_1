...
band = 0
r = 1
x = float(input('Ingrese el monto en Bs a pagar por los suscriptores residenciales: '))
y = float(input('Ingrese el monto en Bs a pagar por los suscriptores comerciales: '))
while r == 1:
...
#RESPUESTA b
    if ts == 1:
        ...
        monto = ce * y
    print('Su consumo electrico es: ', ce)
#RESPUESTA c
    if ts == 1 and ce > 500:
        multa = monto / 2
        monto = monto + multa
#RESPUESTA e
        acum_ce_alco += ce
        cont_ce_alco += 1
        print('Es un suscriptor de alto cosumo su multa es:',multa,'y su total a pagar', monto)
    else:
...
#Respuesta f
    if ts == 1 and ce > 500 and band == 0:
        alco_1 = contrato
        band = 1
    pregunta = int(input('Hay mas suscriptores? \nSI: 1 \nNO: 2'))
    if pregunta == 2:
        r = 2
...
print('El consumo total de la zona de estudio es:', ce_zona)
print(f'El promedio de consumo electrico de los suscriptores de alto consumo es: {prom:6.2f}')
print('El numero de contrato del primer consumidor de alto consumo es:', alco_1)
